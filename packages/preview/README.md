...
The app opens `docs/examples/canvas-v1-shot2-night.json` by default. Use **打开** to load another schema-v1 JSON document. Node moves, node creation, prompt edits (including persisted text-role canvas nodes), and graph connections run through the same browser-safe command core used by the CLI. **导出 JSON** downloads the validated local result; direct project-directory persistence remains a CLI responsibility. `src/demo-assets.js` is an isolated visual fixture map for the checked-in examples.
